...
export default function Converter(obj: OriginalData) {
  ...
  obj["b2_surfacelinemark"].forEach((ob: b2_surfacelinemark, index) => {
    if (ob.properties["Kind"] === "530") {
      ...
      roadmark.NumPoint = 5;
      let point_1 = firstPoint[0] + 0.5 + " " + (firstPoint[1] + 0.5);
      let point_2 = firstPoint[0] - 0.5 + " " + (firstPoint[1] - 0.5);
      let point_3 = lastPoint[0] - 0.5 + " " + (lastPoint[1] - 0.5);
      let point_4 = lastPoint[0] + 0.5 + " " + (lastPoint[1] + 0.5);
      let point_5 = firstPoint[0] + 0.5 + " " + (firstPoint[1] + 0.5);
      roadmark.PointXY = (
        point_1 +
        " " +
        point_2 +
  ...
  obj["c1_trafficlight"].forEach((ob: c1_trafficlight, index) => {
    ...
    roadlight.NumPoint = ob.geometry.coordinates.length;
    roadlight.PointXY = coorTrans(ob.geometry.coordinates).join(" ").trim();
